...
extern ReportPattern * maxEquipmentPerSlotReporter;
extern ReportPattern * privateDropItemsReporter;
extern ReportPattern * drowningReporter;
 ...
 *
 */
void UnitEntity::changeRace(RaceRule * race) {
    raceComposition_->setRule(race);
}
...
bool UnitEntity::hasFollowers() {
    if (stackFollowers_.empty())
        return false;
    else
        return true;
}
...
void UnitEntity::dailyUpdate()
{
    ...
    if (getLocation()->getTerrain()->isAquatic()) {
        if (getLeader() == 0) {
            int stackWeight = 0;
            int swimmingCapacity = 0;
            int flyingCapacity = 0;
            calculateTotalWeight(stackWeight);
            calculateTotalCapacity(swimmingCapacity, swimingMode);
            calculateTotalCapacity(flyingCapacity, flyingMode);

            if ((flyingCapacity < stackWeight) && (swimmingCapacity < stackWeight) && (getContainingConstruction() == 0)) {
                // Stack is drowning.
                if (isStackLeader()) {
                    // unstack all
                    unstackAll();
...
void UnitEntity::drownUnit() {
    ...
    int overWeight;
    if (getCapacity(swimingMode) > getCapacity(flyingMode)) {
        overWeight = getWeight() - getCapacity(swimingMode);
    } else {
        overWeight = getWeight() - getCapacity(flyingMode);
    }
    cout << "SOS! " << print() << " is drowning. Overweight is " << overWeight << "\n";
    // Drop all unequipped items


    dropUnequippedItems();
    if ((getCapacity(flyingMode) < getWeight()) && (getCapacity(swimingMode) < getWeight())) {
        cout << "SOS! " << print() << " still drowning. \n";
        int count = 0;
        int figNum = getFiguresNumber();
        for (int i = 0; i < figNum; i++) {
            int roll = Roll_1Dx(1000);
            if (roll < 250) {
                count++;
            }
        }
        if (count >= figNum) {
            ...
            cout << "SOS! " << print() << " drown. R.I.P." << endl;
            disband();
        } else {
            setFigures(getFiguresNumber() - count);
            addReport(new BinaryMessage(privateDropItemsReporter, this, new RaceElement(getRace(), count)));
            recalculateStats();
        }
    }
}

void UnitEntity::dropUnequippedItems() {
    InventoryIterator iterEquip;
    for (iterEquip = inventory_.getAll().begin(); iterEquip != inventory_.getAll().end();) {
        if ((*iterEquip).getItemNumber() > (*iterEquip).getEquipedNumber()) {
            // drop unequipped items.
            if ((*iterEquip).getEquipedNumber() == 0) {
                cout << " -=- " << print() << " drops " << (*iterEquip).getItemNumber() << " " << (*iterEquip).getItemType()->print() << "\n";
                addReport(new UnaryMessage(privateDropItemsReporter, &(*iterEquip)));
                inventory_.getAll().erase(iterEquip);
                // report
            } else {
                int toDrop = (*iterEquip).getItemNumber() -(*iterEquip).getEquipedNumber();
                (*iterEquip).setItemNumber((*iterEquip).getEquipedNumber());
                addReport(new UnaryMessage(privateDropItemsReporter, new InventoryElement((*iterEquip).getItemType(), toDrop)));
                cout << " -=- " << print() << " drops " << toDrop << " " << (*iterEquip).getItemType()->print() << "\n";
                ++iterEquip;
            }
        } else {
            ++iterEquip;
...
void UnitEntity::privateReport(ReportPrinter &out) {
    ...
    reportEnchantments(getFaction(), out);
...
//               if(skillEl ==0)
//               {
//                cout <<" no requirement for "<<skill->print()<<endl;
//               }
//               else
//               {
//                cout <<"Required "<< skillEl->print()<<" for "<<skill->print()<<endl;
//               }
//
//               }
//        }
 ...
 *
 */
void        UnitEntity::reportEnchantments(FactionEntity * faction, ReportPrinter &out)
{

    enchantments_.report(out);
}
/*
 *
 */
...
STATUS UnitEntity::dataConsistencyCheck() {
    ...
    if (faction_ == 0) {
        cerr << "Incomplete data (no faction reference) for " << print() << endl;
        return IO_ERROR;
    }
    if (getFiguresNumber() == 0) {
        cerr << "Incomplete data (no figures ) for " << print() << endl;
        return IO_ERROR;
    }
...
int UnitEntity::equipItem(ItemRule * item, int num) {
    ...
    // Have already X [slot] equipped.
    if (equipCapacity == inventory_.equipedInSlot(item->getEquipSlot())) {
        addReport(new TertiaryMessage(maxEquipmentPerSlotReporter, this, new IntegerData(equipCapacity), new StringData(item->getEquipSlot()->getName())));
        return 0;
    }
...
void stack(UnitEntity * unit, UnitEntity * newLeader) {
    ...
    if (newLeader->getContainingConstruction() != unit->getContainingConstruction()) {
        unit->recursiveSetContainingConstruction(newLeader->getContainingConstruction());
    }
}
...
/*
 * Moves all  units in the stack out of construction
 */
bool UnitEntity::recursiveSetContainingConstruction(ConstructionEntity * construction) {
    setContainingConstruction(construction);
    recursionCounter++;
    ...
    for (iter = stackFollowers_.begin(); iter != stackFollowers_.end(); iter++) {
        (*iter)->recursiveSetContainingConstruction(construction);
    }
    recursionCounter--;
    return false;
}
...
/*
 * Unstacks all  units in the stack
 */


void UnitEntity::unstackAll() {
    recursionCounter++;
    if (recursionCounter > 100) {
        cout << "Recursion overflow in unstackAll\n";
        return;
    }
    for (int i = 0; i < stackFollowers_.size();) {
        stackFollowers_[i]-> unstackAll();
    }
    unstack();
    recursionCounter--;
...
/*
 * returns the leader of all the stack
 */

UnitEntity * UnitEntity::getStackHead() {
    UnitEntity * last;
    UnitEntity * prev = getPrevStack();
    ...
    while (prev) {
        last = prev;
        prev = last->getPrevStack();
    }
    return last;
}

bool UnitEntity::isFollowingForeignUnit() {
    UnitEntity * last;
    UnitEntity * prev = getPrevStack();
...
void UnitEntity::printStackMembersList(FactionEntity * faction, ReportPrinter &out, bool isOwnStackOnly) {
    if (isOwnStackOnly && faction != faction_) // foreign unit
        return;
    recursionCounter++;
    if (recursionCounter > 100) {
    ...
    //         if(faction == factions["f12"] && location_ == locations["L1"])
    //         {
    //           cout <<"printStackMembersList: "<<print()<<endl;
    //         }
    if (this->getFaction() == faction) {
        privateReport(out);
    } else {
        // Special case when unit is in a foreign stack. It should be visible. Set local observation to unit's stealth
        //
        publicReport(getStealth(), out);
    }
    out.incr_indent();
    StackIterator iter;
    for (iter = stackFollowers_.begin(); iter != stackFollowers_.end(); iter++) {
        (*iter)-> printStackMembersList(faction, out, isOwnStackOnly);
    }
    out.decr_indent();
    recursionCounter--;
}
 ...
 * recursively calculates total capacity of the stack
 */
void UnitEntity::calculateTotalCapacity(int & capacity, MovementVariety * mode) {
    recursionCounter++;
    if (recursionCounter > 100) {
        cout << "Recursion overflow in calculateTotalCapacity of " << getTag() << "\n";
        return;
    }
    capacity += getCapacity(mode);

    StackIterator iter;
    for (iter = stackFollowers_.begin(); iter != stackFollowers_.end(); iter++) {
        (*iter)-> calculateTotalCapacity(capacity, mode);
    }
    recursionCounter--;
}
...
void UnitEntity::recalculateStats() {
    ...
    stats.scaleControlPoints(raceComposition_->getFigures(), raceComposition_->getRace()->getControlPointsFraction());
    // skills
...
void UnitEntity::disband() {
    location_->setPopulation(location_->getPopulation() + getFiguresNumber());
    ...
    UnitEntity * formerLeader = stackFollowingTo_;
    if (unstack()) {

        for (iter = currentFollowers.begin(); iter != currentFollowers.end(); iter++) {
            stack((*iter), formerLeader);
        }
    } else {
        for (iter = currentFollowers.begin(); iter != currentFollowers.end(); iter++) {
            (*iter)->unstack();
            if (moving_) {
                (*iter)->setEntityIndependentlyMoving(moving_);
            }
        }
    }
    // RIP
...
void UnitEntity::exitStack() {
    ...
    if (containingConstruction_ == 0) {
        cout << "Containing Construction is undefined for " << this->print() << endl;
    }
    containingConstruction_->removeUnit(this);
    // cout<< "After removeUnit "<<containingConstruction_<<" / "<<containingConstruction<<endl;
    recalculateStats();
    ...
    if (getLeader()) {
        ...
        //cout<<this->print()<<" followerExitReporter "<<containingConstruction->print()<<" "<< getLeader()->print()<<endl;
    } else {
...
int UnitEntity::getAttackRating() const {
    ...
    if (stats.getDamage() == 0 && stats.getRangedDamage()==0)
        return 0;
    int att = stats.getMelee();
    int dam = stats.getDamage();
    if (stats.getMissile() > att)
    {
        att = stats.getMissile();
        dam = stats.getRangedDamage();
    }
    return (40 + att * 4) * (stats.getHits() + 1) * dam * getFiguresNumber();
}
...
EnchantmentElement * UnitEntity::hasEnchantment(EnchantmentRule * rule)
{
    return enchantments_.has(new EnchantmentElement(rule,1));

}

int UnitEntity::getObservationRating() const
{
   return 0;
}
